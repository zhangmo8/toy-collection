...
function init() {
  renderer.value = new THREE.WebGLRenderer({ canvas: earth.value, antialias: true })
...
function drawLine(startLongitude: number, startLatitude: number, endLongitude: number, endLatitude: number, index: number) {
  ...
  const geometry = new THREE.BoxGeometry().setFromPoints(curvePoints)
